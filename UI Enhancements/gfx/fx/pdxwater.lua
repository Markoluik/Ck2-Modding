...
float3 CalcWaterNormal( float2 uv, float vTimeSpeed )
{
	...
	float2 time2 = -vScaledTime * 0.75f * float2( 0.2f, 0.05f );
	float2 uv1 = vScaleUV * uv;
	float2 uv2 = vScaleUV * uv * 0.5;
...
{
	float3 normal = CalcWaterNormal( Input.uv, vTimeScale * 10.0f );
	...
	float fresnel = saturate( dot( -vEyeDir, normal ) ) * 0.8f;
	...
	float3 outColor = refractiveColor * ( 0.70f - fresnel ) + reflectiveColor * fresnel;
